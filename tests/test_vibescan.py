...
def test_perform_scan(mock_ui_cls, mock_storage_cls, mock_analyzer_cls, temp_source_dir):
...
@patch("viberdash.vibescan.MetricsStorage")
@patch("viberdash.vibescan.DashboardUI")
def test_perform_scan_error_handling(mock_ui_cls, mock_storage_cls, mock_analyzer_cls, temp_source_dir):
...
@patch("viberdash.vibescan.MetricsStorage")
@patch("viberdash.vibescan.DashboardUI")
def test_run_main_loop(mock_ui_cls, mock_storage_cls, mock_analyzer_cls, mock_sleep, temp_source_dir):
...
def test_main_command_no_python_files(mock_console_cls):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "No Python files found" in str(mock_console_cls.return_value.print.call_args_list)
...
""")
        ...
        # Run the command
        runner = CliRunner()
        result = runner.invoke(main, ["--config", str(config_path), "--source-dir", str(tmpdir_path)])
...
@patch("viberdash.vibescan.Console")
@patch("viberdash.vibescan.load_config")
def test_main_command_custom_interval(mock_load_config, mock_console_cls, mock_runner_cls):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Run the command
        runner = CliRunner()
        result = runner.invoke(main, ["--source-dir", str(tmpdir_path), "--interval", "30"])
